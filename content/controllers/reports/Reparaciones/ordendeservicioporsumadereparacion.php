...
function Header()
{
   ...
   $this->Image('../../../../assets/img/logo1.png',10,8,33);
    ...
    // Título
    $this->Cell(160, 10, utf8_decode('ORDENES DE SERVICIO POR VEHICULO') ,2, 0,'C', 0);
        $this->Ln(10);
...
$mysqli = new mysqli("localhost", "root", "", "ut");
        ...
        $resultado = $mysqli->query('SELECT r.id_taller AS id_taller, r.status as status, taller.nombre AS nombre_t, vehiculos.modelo as modelo, taller.rif AS rif, r.nombre AS nombre, r.fecha as fecha,r.descripcion as descripcion, r.id_vehiculo = r.id_vehiculo, vehiculos.id_vehiculo AS id_vehiculo, vehiculos.modelo as modelo, vehiculos.placa AS placa, r.costo AS costo, vehiculos.placa as placa, r.id_reparaciones as id_reparaciones, SUM(costo)costoTotal FROM reparaciones as r INNER JOIN vehiculos ON r.id_vehiculo = vehiculos.id_vehiculo INNER JOIN taller ON r.id_taller= taller.id_taller');
        ...
        $pdf->Cell(20,10, $row['costoTotal'], 1, 0,'C', 0);
